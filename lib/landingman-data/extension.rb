      ...
      def configure_data_files
        ...
        if !options.data1_dir.nil? && !options.data1_dir.blank? then
          ...
          if Dir.exist?(data1_dir) then
            watcher = app.files.watch :global_data1, path: data1_dir, only: ::Middleman::CoreExtensions::Data::DATA_FILE_MATCHER
        ...
        if !options.data2_dir.nil? && !options.data2_dir.blank? then
          ...
          if Dir.exist?(data2_dir) then
            watcher = app.files.watch :global_data2, path: data2_dir, only: ::Middleman::CoreExtensions::Data::DATA_FILE_MATCHER
